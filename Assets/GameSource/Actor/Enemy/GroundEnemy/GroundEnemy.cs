...
public class GroundEnemy : Enemy
{
    ...
    {
        if (!(Time.time - lastPlayerSearchTime > playerSearchIntervalTime) || !GameManager.Instance.isForDos)
            return;
    ...
    IEnumerator attackModel0()
    {
        ...
        for (int i = 0; i < attackModels[index].fireCnt; i++)
        {
            for (int j = 0; j < 2; j++)
            {
                ...
                go = GameManager.Instance.GetCurrentSceneT<InGameScene>().BulletSystem.ServeBullet(BulletCode.enemyBulletM2, fireTransform[j].position);
    ...
    IEnumerator attackModel1()
    {
        ...
        for (int i = 0; i < attackModels[index].fireCnt; i++)
        {
            ...
            {
                go = GameManager.Instance.GetCurrentSceneT<InGameScene>().BulletSystem.ServeBullet(BulletCode.enemyBulletM2, fireTransform[2].position);
    ...
    IEnumerator attackModel2()
    {
        ...
        for (int i = 0; i < attackModels[index].fireCnt; i++)
        {
            ...
            go = GameManager.Instance.GetCurrentSceneT<InGameScene>().BulletSystem.ServeBullet(BulletCode.enemyBulletM4, fireTransform[3].position);
    ...
    IEnumerator attackModel3()
    {
        ...
        go = GameManager.Instance.GetCurrentSceneT<InGameScene>().BulletSystem.ServeBullet(clusterBulletCode, fireTransform[2].position);
    ...
    {
        base.OnDead();
        GameManager.Instance.GetCurrentSceneT<InGameScene>().EffectSystem.ServeEffect(EffectCode.tres, transform.position);
