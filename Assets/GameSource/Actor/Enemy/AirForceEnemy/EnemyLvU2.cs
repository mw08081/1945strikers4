...
public class EnemyLvU2 : AirForceEnemy
{
    ...
    void UpdateAttack()
    {
        if (Time.time - lastAttackTime > attackInterval)
        {
            if(Random.Range(0.0f, 1.0f) >= (1 - attackProbability))
            {
                ...
                {
                    GameObject go = GameManager.Instance.GetCurrentSceneT<InGameScene>().BulletSystem.ServeBullet(BulletCode.enemyBulletM1, bulletSpawnPosition[i].position);
    ...
    void ReturnGameObject()
    {
        ...
        GameManager.Instance.GetCurrentSceneT<InGameScene>().EnemySystem.ReturnEnemy(enemyCode, gameObject);
    }
