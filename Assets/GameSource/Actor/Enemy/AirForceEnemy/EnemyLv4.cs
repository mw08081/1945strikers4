...
public class EnemyLv4 : AirForceEnemy
{
    ...
    IEnumerator AttackModel1()
    {
        for(int i = 0; i < 4; i++)
        {
            //GameObject go = GameManager.Instance.BulletSystem.ServeBullet(BulletCode.enemyBulletM2, bulletSpawnPosition[0].position);
            GameObject go = GameManager.Instance.GetCurrentSceneT<InGameScene>().
    ...
    IEnumerator AttackModel2()
    {
        for(int i = 0; i < 2; i++)
        {
            //GameObject go = GameManager.Instance.BulletSystem.ServeBullet(BulletCode.enemyBulletM2, bulletSpawnPosition[i + 1].position);
            GameObject go = GameManager.Instance.GetCurrentSceneT<InGameScene>()
    ...
    void ReturnGameObject()
    {
        ...
        //GameManager.Instance.EnemySystem.ReturnEnemy(EnemyCode.lv3, gameObject);
        GameManager.Instance.GetCurrentSceneT<InGameScene>().EnemySystem.ReturnEnemy(EnemyCode.lv3, gameObject);
    }
